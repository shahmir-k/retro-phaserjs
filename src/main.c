                        ...
                        if (dl_code) {
                            ...
                            if (strstr(scripts[i].src, "phaser")) {
                                jsc_context_evaluate(g_engine.js_ctx,
                                    ...
                                    // Patch canvasToTexture to ensure _pixelData is up-to-date
                                    "  var _origCanvasToTex = Phaser.Renderer.WebGL.WebGLRenderer.prototype.canvasToTexture;"
                                    "  if (_origCanvasToTex) {"
                                    "    Phaser.Renderer.WebGL.WebGLRenderer.prototype.canvasToTexture = function(srcCanvas, dstTexture, noRepeat, flipY) {"
                                    "      if (srcCanvas && srcCanvas._ctx2d) {"
                                    "        srcCanvas._pixelData = srcCanvas._ctx2d._arrayBuffer;"
                                    "      }"
                                    "      return _origCanvasToTex.call(this, srcCanvas, dstTexture, noRepeat, flipY);"
                                    "    };"
                                    "  }"
                                    "  var _origUpdateCanvasTex = Phaser.Renderer.WebGL.WebGLRenderer.prototype.updateCanvasTexture;"
                                    "  if (_origUpdateCanvasTex) {"
                                    "    Phaser.Renderer.WebGL.WebGLRenderer.prototype.updateCanvasTexture = function(srcCanvas, dstTexture, flipY) {"
                                    "      if (srcCanvas && srcCanvas._ctx2d) {"
                                    "        srcCanvas._pixelData = srcCanvas._ctx2d._arrayBuffer;"
                                    "      }"
                                    "      return _origUpdateCanvasTex.call(this, srcCanvas, dstTexture, flipY);"
                                    "    };"
                                    "  }"
                                    "})();"
                                    , -1);
